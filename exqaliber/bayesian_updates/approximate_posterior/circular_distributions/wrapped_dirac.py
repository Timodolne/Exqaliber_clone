"""Implementation of Wrapped Dirac distribution."""
import numpy as np
...
class WrappedDirac(CircularDistributionBase):
    ...
    def generate_parameters_from_m1(m1: complex) -> tuple[np.ndarray]:
        """Generate distribution parameters beta, gamma from `m1`.

        The distribution generated from these parameters will have the
        same first moment as `m1`.
        ...
        np.ndarray(3)
            Gamma values for wrapped dirac distribution with matched
            `m1`
    ...
    @staticmethod
    def generate_parameters_from_m1_m2(
        m1: complex, m2: complex, a: float = 0.5
    ) -> tuple[np.ndarray]:
        """Generate distribution parameters beta, gamma from m1, m2.

        The distribution generated from these parameters will have the
        same first and second moments as `m1`, `m2` respectively.
        ...
        phi_2 = np.arccos(x_2)
        beta = np.mod(
            mu * np.ones(5) + np.array([-phi_1, phi_1, -phi_2, phi_2, 0]),
            2 * np.pi,
        )
        return beta, gamma
