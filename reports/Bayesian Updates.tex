...
\section{Single shot updates - Gaussian}
...
Now, putting that in to equation \eqref{eq:normalisation_factor} gives
\begin{equation}
    \Pi(Y=y) = \frac{1}{2} \Big[ \int \Pi(\theta) \text{d}\theta + (-1)^y \int \Pi(\theta) \Lambda (\theta) \text{d} \theta \Big].
\end{equation}
The first part equals $1$ by definition. The second part is the expected bias, also by definition, i.e.,
\begin{equation}
...
\end{align}
which apparently (Koh paper, equation 153 on page 25) is an 'identity':
\begin{equation}
    b = e^{-\frac{1}{2}\lambda^2\sigma^2} \cos (\lambda \mu).
\end{equation}
...
\begin{align}
    \Pi'(\theta|Y = y)  &= \frac{\Pi(\theta) \mathcal{L} (y, \theta)}{\Pi(Y=y)}\\
    &= \frac{1}{4\sqrt{2\pi \sigma^2}}\frac{e^{-\frac{1}{2}(\frac{\theta - \mu}{\sigma})^2} ( 1 + (-1)^y \cos(\lambda \theta))}{1 + (-1)^y e^{-\frac{1}{2}\lambda^2\sigma^2} \cos (\lambda \mu)}
\end{align}
